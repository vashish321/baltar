...
router.post('/create', AuthService.requireAuth, async (req, res) => {
  try {
    ...
    if (!projectId || !serviceType || !items || !Array.isArray(items)) {
      return res.status(400).json({ error: 'Missing required fields: projectId, serviceType, items' });
    }
    let invoice;
...
router.post('/auto-generate/:projectId', AuthService.requireAuth, async (req, res) => {
  try {
    const invoice = await InvoiceService.autoGenerateInvoice(req.params.projectId);
    res.status(201).json({ success: true, message: 'Invoice auto-generated successfully', invoice });
  } catch (error) {
    console.error('Error auto-generating invoice:', error);
    res.status(500).json({ error: 'Failed to auto-generate invoice', details: error.message });
  }
});
...
router.get('/:invoiceId', AuthService.requireAuth, async (req, res) => {
  try {
    const invoice = await prisma.invoice.findUnique({
      ...
      include: { invoiceItems: true, client: true, project: true }
    });
...
router.patch('/:invoiceId/status', AuthService.requireAuth, async (req, res) => {
  try {
    ...
    const invoice = await InvoiceService.updateInvoiceStatus(req.params.invoiceId, status);
...
router.post('/:invoiceId/payment', AuthService.requireAuth, async (req, res) => {
  try {
    ...
    const payment = await InvoiceService.recordPayment(req.params.invoiceId, amount, paymentMethod, transactionId);
...
router.get('/', AuthService.requireAuth, async (req, res) => {
  try {
    ...
    if (serviceType) where.project = { serviceType };
    ...
    res.json({ success: true, invoices, pagination: { page: parseInt(page), limit: parseInt(limit), total, pages: Math.ceil(total / limit) } });
  } catch (error) {
...
router.post('/generate', AuthService.requireAuth, async (req, res) => {
  try {
    const { serviceType, customerName, customerEmail, cost, eventType, guestCount, websiteType, projectDescription } = req.body;
    if (!serviceType || !customerName || !customerEmail || !cost) {
      return res.status(400).json({ error: 'Service type, customer name, email, and cost are required' });
    }
    const result = await InvoiceService.generateAndSendInvoice({
      serviceType, customerName, customerEmail, cost: parseFloat(cost),
      eventType, guestCount, websiteType, projectDescription, adminId: req.admin.id
    });
    res.json({ success: true, message: 'Invoice generated and sent successfully', ...result });
  } catch (error) {
